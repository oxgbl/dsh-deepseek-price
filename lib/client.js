 ...
 *   高峰时段：周一至周五 09:00–12:00、14:00–18:00；周末（周六/周日）全天空闲；
 *   空闲时段价格为高峰时段价格的一半。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 下一个时段边界（周末感知）。返回 { at, nextPeriod, daysAhead }：
    ...
    function nextBoundary(date) {
      ...
      if (!(weekday === 0 || weekday === 6)) {
        for (var i = 0; i < boundaries.length; i++) {
          if (minutes < boundaries[i].at) return { at: boundaries[i].at, nextPeriod: boundaries[i].nextPeriod, daysAhead: 0 };
        }
        // 工作日 18:00 后：周五 → 下周一 09:00；其余 → 次日 09:00
        return { at: 540, nextPeriod: "peak", daysAhead: weekday === 5 ? 3 : 1 };
      }
      // 周末：下一个高峰是下周一 09:00（周六 +2 天，周日 +1 天）
      return { at: 540, nextPeriod: "peak", daysAhead: weekday === 6 ? 2 : 1 };
    }
    ...
    function boundaryText(date, next) {
      var time = formatClock(next.at);
      if (next.daysAhead <= 0) return time;
      var target = (beijingWeekday(date) + next.daysAhead) % 7;
      return WEEKDAY_LABELS[target] + " " + time;
    }

    function summaryLine(date) {
      var st = periodAt(date);
      var next = nextBoundary(date);
      var minutesLeft = next.daysAhead * 24 * 60 + next.at - st.minutes;
    ...
    function buildTitle(date) {
      ...
      var lines = [
        ...
        "高峰时段：周一至周五 09:00–12:00、14:00–18:00（周末全天空闲，价格减半）",
        "",
    ...
    function buildPanel(date) {
      ...
      var lines = [
        ...
        react.createElement("div", { key: "w" }, "高峰时段：周一至周五 09:00–12:00、14:00–18:00（周末全天空闲，价格减半）"),
